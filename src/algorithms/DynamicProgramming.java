...
public class DynamicProgramming {
    ...
    public void performDPsize(QueryGraph queryGraph, Boolean allowCP){
        ...
        List<String> orderedRelationName = orderRelationNames(queryGraph.relations);
        dpTable.put(1, queryGraph.relations);
        printTableEntry(dpTable.get(1), 1);
        int size = 2;
        while(size <= orderedRelationName.size()){
            dpTable.put(size, new HashMap<>());
            List<String> relevantRelations = new ArrayList<>();
            loopThroughPermutations(0, size, orderedRelationName, relevantRelations, dpTable, queryGraph);
    ...
    public List<String> orderRelationNames(Map<String, Relation> printTableEntry){
        List<String> orderedRelationNames = new ArrayList<>();
        for(String key: printTableEntry.keySet()){
            orderedRelationNames.add(key);
        }
        return orderedRelationNames;
    }
    ...
    public void performDPsub(){
        // print the order of DP sub
    }

    public void performDPccp(){
